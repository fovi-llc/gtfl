...
Two real-life examples for an application of GTFL can be found at https://www.martin-loetzsch.de/gtfl/application-example-1 and https://www.martin-loetzsch.de/gtfl/application-example-2. These are debug traces of linguistic parsing and production in the [Fluid Construction Grammar](https://fcg-net.org) (FCG) framework (more examples [here](https://github.com/dwarfmaster/Babel2/tree/master/systems/fcg/anti-unification/static-html)). Such traces help the developers FCG to understand and debug their programs and they help users of FCG to see what their linguistic rules are doing. By encapsulating visualizations for representations in expandable html elements, the complete trace fits into one browser window and still includes every little detail and intermediate processing step of the involved FCG algorithms (which would be thousands of pages debugging output to the listener).

#### GitHub Copy

[jimwhite@](https://github.com/jimwhite) copied the archive https://martin-loetzsch.de/gtfl/gtfl.tar.gz with version 0.1.3 on June 11, 2024 from the homepage https://www.martin-loetzsch.de/gtfl/ to https://github.com/fovi-llc/gtfl to ensure this nifty LISP tool isn't lost.  This README is a partial conversion and some tweaks of the [index.html](https://html-preview.github.io/?url=https://github.com/fovi-llc/gtfl/blob/main/index.html) file which has all the details formatted by GTFL itself.  I've updated (most of) the HTTP URLs to HTTPS.  The application examples in the html dir don't work, not only are there CORS issues with the .js file but there are a bunch of XML files needed to make them work.
